...
var invariant__default = _interopDefault(invariant);
...
function validateOperation(operation) {
    ...
    for (var _i = 0, _a = Object.keys(operation); _i < _a.length; _i++) {
        ...
        if (OPERATION_FIELDS.indexOf(key) < 0) {
            throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(33) : new invariant.InvariantError("illegal argument: " + key);
        }
    }
    return operation;
}
...
function getFragmentQueryDocument(document, fragmentName) {
    ...
    document.definitions.forEach(function (definition) {
        if (definition.kind === 'OperationDefinition') {
            throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(39) : new invariant.InvariantError("Found a " + definition.operation + " operation" + (definition.name ? " named '" + definition.name.value + "'" : '') + ". " +
    ...
    if (typeof actualFragmentName === 'undefined') {
        process.env.NODE_ENV === "production" ? invariant.invariant(fragments.length === 1, 40) : invariant.invariant(fragments.length === 1, "Found " + fragments.length + " fragments. `fragmentName` must be provided when there is not exactly 1 fragment.");
...
function getFragmentFromSelection(selection, fragmentMap) {
    switch (selection.kind) {
        ...
        case 'FragmentSpread': {
            ...
            process.env.NODE_ENV === "production" ? invariant.invariant(fragment, 41) : invariant.invariant(fragment, "No fragment named " + selection.name.value + ".");
            return fragment;
        }
        default:
            return null;
...
function valueToObjectRepresentation(argObj, name, value, variables) {
    if (isIntValue(value) || isFloatValue(value)) {
        ...
        throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(34) : new invariant.InvariantError("The inline argument \"" + name.value + "\" of kind \"" + value.kind + "\"" +
...
function checkDocument(doc) {
    process.env.NODE_ENV === "production" ? invariant.invariant(doc && doc.kind === 'Document', 25) : invariant.invariant(doc && doc.kind === 'Document', "Expecting a parsed GraphQL document. Perhaps you need to wrap the query string in a \"gql\" tag? http://docs.apollostack.com/apollo-client/core.html#gql");
    var operations = doc.definitions
        .filter(function (d) { return d.kind !== 'FragmentDefinition'; })
        .map(function (definition) {
        if (definition.kind !== 'OperationDefinition') {
            throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(26) : new invariant.InvariantError("Schema type definitions not allowed in queries. Found: \"" + definition.kind + "\"");
        }
        return definition;
    });
    process.env.NODE_ENV === "production" ? invariant.invariant(operations.length <= 1, 27) : invariant.invariant(operations.length <= 1, "Ambiguous GraphQL document: contains " + operations.length + " operations");
    return doc;
}
...
function getQueryDefinition(doc) {
    ...
    process.env.NODE_ENV === "production" ? invariant.invariant(queryDef && queryDef.operation === 'query', 28) : invariant.invariant(queryDef && queryDef.operation === 'query', 'Must contain a query definition.');
    return queryDef;
}
function getFragmentDefinition(doc) {
    process.env.NODE_ENV === "production" ? invariant.invariant(doc.kind === 'Document', 29) : invariant.invariant(doc.kind === 'Document', "Expecting a parsed GraphQL document. Perhaps you need to wrap the query string in a \"gql\" tag? http://docs.apollostack.com/apollo-client/core.html#gql");
    process.env.NODE_ENV === "production" ? invariant.invariant(doc.definitions.length <= 1, 30) : invariant.invariant(doc.definitions.length <= 1, 'Fragment must have exactly one definition.');
    var fragmentDef = doc.definitions[0];
    process.env.NODE_ENV === "production" ? invariant.invariant(fragmentDef.kind === 'FragmentDefinition', 31) : invariant.invariant(fragmentDef.kind === 'FragmentDefinition', 'Must be a fragment definition.');
    return fragmentDef;
}
function getMainDefinition(queryDoc) {
    ...
    throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(32) : new invariant.InvariantError('Expected a parsed GraphQL query with a query, mutation, subscription, or a fragment.');
}
...
var ApolloLink = (function () {
    ...
    ApolloLink.concat = function (first, second) {
        ...
        if (isTerminating(firstLink)) {
            process.env.NODE_ENV === "production" || invariant.invariant.warn(new LinkError("You are calling concat on a terminating link, which will have no effect", firstLink));
            return firstLink;
        }
    ...
    ApolloLink.prototype.request = function (operation, forward) {
        throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(4) : new invariant.InvariantError('request is not implemented');
    };
...
function shouldInclude(_a, variables) {
    ...
    return getInclusionDirectives(directives).every(function (_a) {
        ...
        if (ifArgument.value.kind === 'Variable') {
            ...
            process.env.NODE_ENV === "production" ? invariant.invariant(evaledValue !== void 0, 42) : invariant.invariant(evaledValue !== void 0, "Invalid variable referenced in @" + directive.name.value + " directive.");
        }
        else {
...
function getInclusionDirectives(directives) {
    ...
    if (directives && directives.length) {
        directives.forEach(function (directive) {
            ...
            process.env.NODE_ENV === "production" ? invariant.invariant(directiveArguments && directiveArguments.length === 1, 43) : invariant.invariant(directiveArguments && directiveArguments.length === 1, "Incorrect number of arguments for the @" + directiveName + " directive.");
            var ifArgument = directiveArguments[0];
            process.env.NODE_ENV === "production" ? invariant.invariant(ifArgument.name && ifArgument.name.value === 'if', 44) : invariant.invariant(ifArgument.name && ifArgument.name.value === 'if', "Invalid argument for the @" + directiveName + " directive.");
            var ifValue = ifArgument.value;
            process.env.NODE_ENV === "production" ? invariant.invariant(ifValue &&
                (ifValue.kind === 'Variable' || ifValue.kind === 'BooleanValue'), 45) : invariant.invariant(ifValue &&
    ...
    test: function (directive) {
        ...
        if (willRemove) {
            if (!directive.arguments ||
                ...
                process.env.NODE_ENV === "production" || invariant.invariant.warn('Removing an @connection directive even though it does not have a key. ' +
                ...
                execute = function (selection) { return tslib.__awaiter(_this, void 0, void 0, function () {
                    ...
                    return tslib.__generator(this, function (_a) {
                        ...
                        if (isInlineFragment(selection)) {
                            ...
                            process.env.NODE_ENV === "production" ? invariant.invariant(fragment, 24) : invariant.invariant(fragment, "No fragment named " + selection.name.value);
                        }
...
var ObservableQuery = (function (_super) {
    ...
    ObservableQuery.prototype.refetch = function (variables) {
        ...
        if (fetchPolicy === 'cache-only') {
            return Promise.reject(process.env.NODE_ENV === "production" ? new invariant.InvariantError(3) : new invariant.InvariantError('cache-only fetchPolicy option should not be used together with query refetch.'));
        }
    ...
    ObservableQuery.prototype.subscribeToMore = function (options) {
        ...
        var subscription = this.queryManager
            ...
            error: function (err) {
                ...
                process.env.NODE_ENV === "production" || invariant.invariant.error('Unhandled GraphQL subscription error', err);
            },
        });
...
function defaultSubscriptionObserverErrorCallback(error) {
    process.env.NODE_ENV === "production" || invariant.invariant.error('Unhandled error', error.message, error.stack);
}
...
var QueryManager = (function () {
    ...
    QueryManager.prototype.stop = function () {
        ...
        this.fetchQueryRejectFns.forEach(function (reject) {
            reject(process.env.NODE_ENV === "production" ? new invariant.InvariantError(14) : new invariant.InvariantError('QueryManager stopped while query was in flight'));
        });
    };
    QueryManager.prototype.mutate = function (_a) {
        ...
        return tslib.__awaiter(this, void 0, void 0, function () {
            ...
            return tslib.__generator(this, function (_f) {
                switch (_f.label) {
                    case 0:
                        process.env.NODE_ENV === "production" ? invariant.invariant(mutation, 15) : invariant.invariant(mutation, 'mutation option is required. You must specify your GraphQL document in the mutation option.');
                        process.env.NODE_ENV === "production" ? invariant.invariant(!fetchPolicy || fetchPolicy === 'no-cache', 16) : invariant.invariant(!fetchPolicy || fetchPolicy === 'no-cache', "Mutations only support a 'no-cache' fetchPolicy. If you don't want to disable the cache, remove your fetchPolicy setting to proceed with the default mutation behavior.");
    ...
    QueryManager.prototype.queryListenerForObserver = function (queryId, observer) {
        return function (info) {
            ...
            if (diff.complete ||
                ...
                process.env.NODE_ENV === "production" || invariant.invariant.warn("Missing cache result fields: " + diff.missing.map(function (m) { return m.path.join('.'); }).join(', '), diff.missing);
            }
        };
    };
    ...
    QueryManager.prototype.query = function (options) {
        var _this = this;
        process.env.NODE_ENV === "production" ? invariant.invariant(options.query, 17) : invariant.invariant(options.query, 'query option is required. You must specify your GraphQL document ' +
            'in the query option.');
        process.env.NODE_ENV === "production" ? invariant.invariant(options.query.kind === 'Document', 18) : invariant.invariant(options.query.kind === 'Document', 'You must wrap the query string in a "gql" tag.');
        process.env.NODE_ENV === "production" ? invariant.invariant(!options.returnPartialData, 19) : invariant.invariant(!options.returnPartialData, 'returnPartialData option only supported on watchQuery.');
        process.env.NODE_ENV === "production" ? invariant.invariant(!options.pollInterval, 20) : invariant.invariant(!options.pollInterval, 'pollInterval option only supported on watchQuery.');
        return new Promise(function (resolve, reject) {
    ...
    QueryManager.prototype.clearStore = function () {
        this.fetchQueryRejectFns.forEach(function (reject) {
            reject(process.env.NODE_ENV === "production" ? new invariant.InvariantError(21) : new invariant.InvariantError('Store reset while query was in flight (not completed in link chain)'));
        });
    ...
    QueryManager.prototype.getQueryWithPreviousResult = function (queryIdOrObservable) {
        ...
        if (typeof queryIdOrObservable === 'string') {
            ...
            process.env.NODE_ENV === "production" ? invariant.invariant(foundObservableQuery, 22) : invariant.invariant(foundObservableQuery, "ObservableQuery with this id doesn't exist: " + queryIdOrObservable);
    ...
    QueryManager.prototype.startPollingQuery = function (options, queryId) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(pollInterval, 23) : invariant.invariant(pollInterval, 'Attempted to start a polling query without a polling interval.');
        if (!this.ssrMode) {
...
var serializeFetchParameter = function (p, label) {
    ...
    try {
        ...
        var parseError = process.env.NODE_ENV === "production" ? new invariant.InvariantError(5) : new invariant.InvariantError("Network request failed. " + label + " is not serializable: " + e.message);
...
var checkFetcher = function (fetcher) {
    if (!fetcher && typeof fetch === 'undefined') {
        ...
        if (typeof window === 'undefined')
            library = 'node-fetch';
        throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(6) : new invariant.InvariantError('"fetch" has not been found globally and no fetcher has been ' +
...
var ApolloClient = (function () {
    function ApolloClient(options) {
        ...
        if (!cache) {
            throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(1) : new invariant.InvariantError("To initialize Apollo Client, you must specify a 'cache' property " +
    ...
    ApolloClient.prototype.query = function (options) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(options.fetchPolicy !== 'cache-and-network', 2) : invariant.invariant(options.fetchPolicy !== 'cache-and-network', 'The cache-and-network fetchPolicy does not work with client.query, because ' +
...
var Policies = (function () {
    ...
    Policies.prototype.setRootTypename = function (which, typename) {
        ...
        if (typename !== old) {
            process.env.NODE_ENV === "production" ? invariant.invariant(!old || old === which, 35) : invariant.invariant(!old || old === which, "Cannot change root " + which + " __typename more than once");
    ...
    Policies.prototype.readField = function (objectOrReference, nameOrField, context, typename) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(objectOrReference, 36) : invariant.invariant(objectOrReference, "Must provide an object or Reference when calling Policies#readField");
        var policies = this;
    ...
    return {
        ...
        mergeObjects: function (existing, incoming) {
            if (Array.isArray(existing) || Array.isArray(incoming)) {
                throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(37) : new invariant.InvariantError("Cannot automatically merge arrays");
            }
...
function computeKeyObject(response, specifier, aliasMap) {
    ...
    specifier.forEach(function (s) {
        if (Array.isArray(s)) {
            ...
            process.env.NODE_ENV === "production" ? invariant.invariant(hasOwn.call(response, responseName), 38) : invariant.invariant(hasOwn.call(response, responseName), "Missing field '" + responseName + "' while computing key fields");
...
var StoreReader = (function () {
    ...
    StoreReader.prototype.executeSelectionSet = function (_a) {
        ...
        if (isReference(objectOrReference) &&
            ...
            return {
                result: {},
                missing: [missingFromInvariant(process.env.NODE_ENV === "production" ? new invariant.InvariantError(49) : new invariant.InvariantError("Dangling reference to missing " + objectOrReference.__ref + " object"), context)],
            };
        }
            ...
            if (isField(selection)) {
                ...
                if (fieldValue === void 0) {
                    if (!addTypenameToDocument.added(selection)) {
                        getMissing().push(missingFromInvariant(process.env.NODE_ENV === "production" ? new invariant.InvariantError(50) : new invariant.InvariantError("Can't find field '" + selection.name.value + "' on " + (isReference(objectOrReference)
                ...
                invariant.invariant(context.path.pop() === resultName);
            }
            else {
                var fragment = void 0;
                if (isInlineFragment(selection)) {
                    ...
                    process.env.NODE_ENV === "production" ? invariant.invariant(fragment = fragmentMap[selection.name.value], 51) : invariant.invariant(fragment = fragmentMap[selection.name.value], "No fragment named " + selection.name.value);
                }
    ...
    StoreReader.prototype.executeSubSelectedArray = function (_a) {
        ...
        function handleMissing(childResult, i) {
            ...
            invariant.invariant(context.path.pop() === i);
    ...
    if (!field.selectionSet) {
        ...
        workSet_1.forEach(function (value) {
            if (value && typeof value === "object") {
                process.env.NODE_ENV === "production" ? invariant.invariant(!isReference(value), 52) : invariant.invariant(!isReference(value), "Missing selection set for object of type " + getTypenameFromStoreObject(store, value) + " returned for query field " + field.name.value);
        ...
        workSet.forEach(function (selection) {
            ...
            if (isField(selection)) {
                ...
                if (typeof value !== 'undefined') {
                    ...
                    throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(48) : new invariant.InvariantError("Missing field '" + resultFieldKey + "' in " + JSON.stringify(result, null, 2).substring(0, 100));
                }
            }
            else {
...
var ApolloProvider = function (_a) {
    ...
    return React.createElement(ApolloContext.Consumer, null, function (context) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(context.client, 7) : invariant.invariant(context.client, 'ApolloProvider was not passed a client instance. Make ' +
...
var MockLink = (function (_super) {
    ...
    MockLink.prototype.request = function (operation) {
        ...
        var response = (this.mockedResponsesByKey[key] || []).find(function (res, index) {
            ...
            if (!equal.equal(stringify(requestVariables), stringify(mockedResponseVariables))) {
                return false;
            }
            responseIndex = index;
            return true;
        ...
        invariant__default(response, "mocked response is required");
    ...
    MockLink.prototype.normalizeMockedResponse = function (mockedResponse) {
        ...
        invariant__default(queryWithoutConnection, "query is required");
