...
var equal__default = _interopDefault(equal);
...
function getFragmentQueryDocument(document, fragmentName) {
    ...
    document.definitions.forEach(function (definition) {
        if (definition.kind === 'OperationDefinition') {
            throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(39) : new tsInvariant.InvariantError("Found a " + definition.operation + " operation" + (definition.name ? " named '" + definition.name.value + "'" : '') + ". " +
    ...
    if (typeof actualFragmentName === 'undefined') {
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(fragments.length === 1, 40) : tsInvariant.invariant(fragments.length === 1, "Found " + fragments.length + " fragments. `fragmentName` must be provided when there is not exactly 1 fragment.");
...
function getFragmentFromSelection(selection, fragmentMap) {
    switch (selection.kind) {
        ...
        case 'FragmentSpread': {
            ...
            process.env.NODE_ENV === "production" ? tsInvariant.invariant(fragment, 41) : tsInvariant.invariant(fragment, "No fragment named " + selection.name.value + ".");
            return fragment;
        }
        default:
            return null;
...
function valueToObjectRepresentation(argObj, name, value, variables) {
    if (isIntValue(value) || isFloatValue(value)) {
        ...
        throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(34) : new tsInvariant.InvariantError("The inline argument \"" + name.value + "\" of kind \"" + value.kind + "\"" +
...
function validateOperation(operation) {
    ...
    for (var _i = 0, _a = Object.keys(operation); _i < _a.length; _i++) {
        ...
        if (OPERATION_FIELDS.indexOf(key) < 0) {
            throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(33) : new tsInvariant.InvariantError("illegal argument: " + key);
        }
    }
    return operation;
}
...
function checkDocument(doc) {
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(doc && doc.kind === 'Document', 25) : tsInvariant.invariant(doc && doc.kind === 'Document', "Expecting a parsed GraphQL document. Perhaps you need to wrap the query string in a \"gql\" tag? http://docs.apollostack.com/apollo-client/core.html#gql");
    var operations = doc.definitions
        .filter(function (d) { return d.kind !== 'FragmentDefinition'; })
        .map(function (definition) {
        if (definition.kind !== 'OperationDefinition') {
            throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(26) : new tsInvariant.InvariantError("Schema type definitions not allowed in queries. Found: \"" + definition.kind + "\"");
        }
        return definition;
    });
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(operations.length <= 1, 27) : tsInvariant.invariant(operations.length <= 1, "Ambiguous GraphQL document: contains " + operations.length + " operations");
    return doc;
}
...
function getQueryDefinition(doc) {
    ...
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(queryDef && queryDef.operation === 'query', 28) : tsInvariant.invariant(queryDef && queryDef.operation === 'query', 'Must contain a query definition.');
    return queryDef;
}
function getFragmentDefinition(doc) {
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(doc.kind === 'Document', 29) : tsInvariant.invariant(doc.kind === 'Document', "Expecting a parsed GraphQL document. Perhaps you need to wrap the query string in a \"gql\" tag? http://docs.apollostack.com/apollo-client/core.html#gql");
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(doc.definitions.length <= 1, 30) : tsInvariant.invariant(doc.definitions.length <= 1, 'Fragment must have exactly one definition.');
    var fragmentDef = doc.definitions[0];
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(fragmentDef.kind === 'FragmentDefinition', 31) : tsInvariant.invariant(fragmentDef.kind === 'FragmentDefinition', 'Must be a fragment definition.');
    return fragmentDef;
}
function getMainDefinition(queryDoc) {
    ...
    throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(32) : new tsInvariant.InvariantError('Expected a parsed GraphQL query with a query, mutation, subscription, or a fragment.');
}
...
var ApolloLink = (function () {
    ...
    ApolloLink.prototype.request = function (operation, forward) {
        throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(4) : new tsInvariant.InvariantError('request is not implemented');
    };
...
var ObservableQuery = (function (_super) {
                    ...
                    if (hasClientExports) {
                        queryManager.getLocalState().addExportedVariables(query, variables).then(function (variables) {
                            ...
                            if (!result.loading &&
                                ...
                                !equal.equal(previousVariables, variables)) {
    ...
    ObservableQuery.prototype.getCurrentResult = function () {
        ...
        if (queryStoreValue) {
            ...
            if (queryStoreValue.variables) {
                this.options.variables = tslib.__assign(tslib.__assign({}, this.options.variables), queryStoreValue.variables);
            }
    ...
    ObservableQuery.prototype.isDifferentFromLastResult = function (newResult) {
        return !equal.equal(this.lastResultSnapshot, newResult);
    };
    ...
    ObservableQuery.prototype.refetch = function (variables) {
        ...
        return this.queryManager.fetchQuery(this.queryId, tslib.__assign(tslib.__assign({}, this.options), { fetchPolicy: fetchPolicy }), exports.FetchType.refetch);
    };
    ObservableQuery.prototype.fetchMore = function (fetchMoreOptions) {
        var _this = this;
        var combinedOptions = tslib.__assign(tslib.__assign({}, (fetchMoreOptions.query ? fetchMoreOptions : tslib.__assign(tslib.__assign(tslib.__assign({}, this.options), fetchMoreOptions), { variables: tslib.__assign(tslib.__assign({}, this.options.variables), fetchMoreOptions.variables) }))), { fetchPolicy: 'network-only' });
    ...
    ObservableQuery.prototype.setOptions = function (opts) {
        ...
        return this.setVariables(this.options.variables, fetchPolicyChanged || !equal.equal(this.variables, oldVariables));
    };
    ObservableQuery.prototype.setVariables = function (variables, tryFetch) {
        if (tryFetch === void 0) { tryFetch = !equal.equal(this.variables, variables); }
        this.isTornDown = false;
        if (!tryFetch) {
...
var serializeFetchParameter = function (p, label) {
    ...
    try {
        ...
        var parseError = process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(5) : new tsInvariant.InvariantError("Network request failed. " + label + " is not serializable: " + e.message);
...
var checkFetcher = function (fetcher) {
    if (!fetcher && typeof fetch === 'undefined') {
        ...
        if (typeof window === 'undefined')
            library = 'node-fetch';
        throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(6) : new tsInvariant.InvariantError('"fetch" has not been found globally and no fetcher has been ' +
                ...
                execute = function (selection) { return tslib.__awaiter(_this, void 0, void 0, function () {
                    ...
                    return tslib.__generator(this, function (_a) {
                        ...
                        if (isInlineFragment(selection)) {
                            ...
                            process.env.NODE_ENV === "production" ? tsInvariant.invariant(fragment, 24) : tsInvariant.invariant(fragment, "No fragment named " + selection.name.value);
                        }
...
var QueryInfo = (function () {
    ...
    QueryInfo.prototype.shouldNotify = function () {
        ...
        if (isNetworkRequestInFlight(this.networkStatus)) {
            ...
            var shouldNotifyIfLoading = returnPartialData ||
                this.networkStatus === exports.NetworkStatus.setVariables ||
    ...
    QueryInfo.prototype.markResult = function (result, _a, allowCacheWrite, makeReady) {
        ...
        if (makeReady) {
            ...
            this.networkStatus = exports.NetworkStatus.ready;
        }
    };
...
var QueryManager = (function () {
    ...
    QueryManager.prototype.stop = function () {
        ...
        this.fetchQueryRejectFns.forEach(function (reject) {
            reject(process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(14) : new tsInvariant.InvariantError('QueryManager stopped while query was in flight'));
        });
    };
            ...
            return tslib.__generator(this, function (_d) {
                switch (_d.label) {
                    ...
                    case 2:
                        ...
                        this.setNetStatus(fetchMoreForQueryId, exports.NetworkStatus.fetchMore);
                        if (shouldFetch) {
                            this.broadcastQueries();
                            networkResult = this.fetchRequest({
                                ...
                                if (isApolloError(error)) {
                                    ...
                                    if (requestId >= _this.getQuery(queryId).lastRequestId) {
                                        ...
                                        _this.setNetStatus(fetchMoreForQueryId, exports.NetworkStatus.ready);
                                        _this.dirty(queryId);
                        ...
                        this.setNetStatus(queryId, shouldFetch ? queryInfo.networkStatus : exports.NetworkStatus.ready);
                        this.dirty(queryId);
                        this.dirty(fetchMoreForQueryId);
    ...
    QueryManager.prototype.setNetStatus = function (queryId, status) {
        ...
        if (queryInfo) {
            ...
            if (status === exports.NetworkStatus.ready) {
    ...
    QueryManager.prototype.query = function (options) {
        var _this = this;
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(options.query, 17) : tsInvariant.invariant(options.query, 'query option is required. You must specify your GraphQL document ' +
            'in the query option.');
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(options.query.kind === 'Document', 18) : tsInvariant.invariant(options.query.kind === 'Document', 'You must wrap the query string in a "gql" tag.');
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(!options.returnPartialData, 19) : tsInvariant.invariant(!options.returnPartialData, 'returnPartialData option only supported on watchQuery.');
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(!options.pollInterval, 20) : tsInvariant.invariant(!options.pollInterval, 'pollInterval option only supported on watchQuery.');
        return new Promise(function (resolve, reject) {
    ...
    QueryManager.prototype.clearStore = function () {
        this.fetchQueryRejectFns.forEach(function (reject) {
            reject(process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(21) : new tsInvariant.InvariantError('Store reset while query was in flight (not completed in link chain)'));
        });
    ...
    QueryManager.prototype.getQueryWithPreviousResult = function (queryIdOrObservable) {
        ...
        if (typeof queryIdOrObservable === 'string') {
            ...
            process.env.NODE_ENV === "production" ? tsInvariant.invariant(foundObservableQuery, 22) : tsInvariant.invariant(foundObservableQuery, "ObservableQuery with this id doesn't exist: " + queryIdOrObservable);
    ...
    QueryManager.prototype.startPollingQuery = function (options, queryId) {
        ...
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(pollInterval, 23) : tsInvariant.invariant(pollInterval, 'Attempted to start a polling query without a polling interval.');
        if (!this.ssrMode) {
...
var ApolloClient = (function () {
    function ApolloClient(options) {
        ...
        if (!cache) {
            throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(1) : new tsInvariant.InvariantError("To initialize Apollo Client, you must specify a 'cache' property " +
...
var Policies = (function () {
    ...
    Policies.prototype.setRootTypename = function (which, typename) {
        ...
        if (typename !== old) {
            process.env.NODE_ENV === "production" ? tsInvariant.invariant(!old || old === which, 35) : tsInvariant.invariant(!old || old === which, "Cannot change root " + which + " __typename more than once");
    ...
    Policies.prototype.readField = function (objectOrReference, nameOrField, context, typename) {
        ...
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(objectOrReference, 36) : tsInvariant.invariant(objectOrReference, "Must provide an object or Reference when calling Policies#readField");
        var policies = this;
    ...
    return {
        ...
        mergeObjects: function (existing, incoming) {
            if (Array.isArray(existing) || Array.isArray(incoming)) {
                throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(37) : new tsInvariant.InvariantError("Cannot automatically merge arrays");
            }
...
function computeKeyObject(response, specifier, aliasMap) {
    ...
    specifier.forEach(function (s) {
        if (Array.isArray(s)) {
            ...
            process.env.NODE_ENV === "production" ? tsInvariant.invariant(hasOwn.call(response, responseName), 38) : tsInvariant.invariant(hasOwn.call(response, responseName), "Missing field '" + responseName + "' while computing key fields");
...
var StoreReader = (function () {
    ...
    StoreReader.prototype.executeSelectionSet = function (_a) {
        ...
        if (isReference(objectOrReference) &&
            ...
            return {
                result: {},
                missing: [missingFromInvariant(process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(49) : new tsInvariant.InvariantError("Dangling reference to missing " + objectOrReference.__ref + " object"), context)],
            };
        }
            ...
            if (isField(selection)) {
                ...
                if (fieldValue === void 0) {
                    if (!addTypenameToDocument.added(selection)) {
                        getMissing().push(missingFromInvariant(process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(50) : new tsInvariant.InvariantError("Can't find field '" + selection.name.value + "' on " + (isReference(objectOrReference)
                ...
                if (isInlineFragment(selection)) {
                    ...
                    process.env.NODE_ENV === "production" ? tsInvariant.invariant(fragment = fragmentMap[selection.name.value], 51) : tsInvariant.invariant(fragment = fragmentMap[selection.name.value], "No fragment named " + selection.name.value);
                }
    ...
    if (!field.selectionSet) {
        ...
        workSet_1.forEach(function (value) {
            if (value && typeof value === "object") {
                process.env.NODE_ENV === "production" ? tsInvariant.invariant(!isReference(value), 52) : tsInvariant.invariant(!isReference(value), "Missing selection set for object of type " + getTypenameFromStoreObject(store, value) + " returned for query field " + field.name.value);
        ...
        workSet.forEach(function (selection) {
            ...
            if (isField(selection)) {
                ...
                if (typeof value !== 'undefined') {
                    ...
                    throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(48) : new tsInvariant.InvariantError("Missing field '" + resultFieldKey + "' in " + JSON.stringify(result, null, 2).substring(0, 100));
                }
            }
            else {
...
var ApolloProvider = function (_a) {
    ...
    return React.createElement(ApolloContext.Consumer, null, function (context) {
        ...
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(context.client, 7) : tsInvariant.invariant(context.client, 'ApolloProvider was not passed a client instance. Make ' +
...
var ApolloConsumer = function (props) {
    ...
    var ApolloContext = getApolloContext();
    return React.createElement(ApolloContext.Consumer, null, function (context) {
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(context && context.client, 9) : tsInvariant.invariant(context && context.client, 'Could not find "client" in the context of ApolloConsumer. ' +
...
function parser(document) {
    ...
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(!!document && !!document.kind, 10) : tsInvariant.invariant(!!document && !!document.kind, "Argument of " + document + " passed to parser was not a valid GraphQL " +
    ...
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(!fragments.length ||
        (queries.length || mutations.length || subscriptions.length), 11) : tsInvariant.invariant(!fragments.length ||
        (queries.length || mutations.length || subscriptions.length), "Passing only a fragment to 'graphql' is not yet supported. " +
        "You must include a query, subscription or mutation as well");
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(queries.length + mutations.length + subscriptions.length <= 1, 12) : tsInvariant.invariant(queries.length + mutations.length + subscriptions.length <= 1, "react-apollo only supports a query, subscription, or a mutation per HOC. " +
    ...
    var definitions = queries.length
        ...
        : mutations.length
            ? mutations
            : subscriptions;
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(definitions.length === 1, 13) : tsInvariant.invariant(definitions.length === 1, "react-apollo only supports one definition per HOC. " + document + " had " +
...
var OperationData = (function () {
    ...
    OperationData.prototype.setOptions = function (newOptions, storePrevious) {
        ...
        if (storePrevious && !equal.equal(this.options, newOptions)) {
    ...
    OperationData.prototype.refreshClient = function () {
        ...
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(!!client, 46) : tsInvariant.invariant(!!client, 'Could not find "client" in the context or passed in as an option. ' +
    ...
    OperationData.prototype.verifyDocumentType = function (document, type) {
        ...
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(operation.type === type, 47) : tsInvariant.invariant(operation.type === type, "Running a " + requiredOperationName + " requires a graphql " +
...
var QueryData = (function (_super) {
    ...
    function QueryData(_a) {
        ...
        _this.getQueryResult = function () {
            ...
            if (options.skip) {
                ...
                result = tslib.__assign(tslib.__assign({}, result), { loading: loading,
                    networkStatus: networkStatus,
                ...
                if (loading) {
                    ...
                    result.data =
                        previousData && data
                            ? tslib.__assign(tslib.__assign({}, previousData), data) : previousData || data;
                }
                else if (error) {
                    Object.assign(result, {
    ...
    QueryData.prototype.updateObservableQuery = function () {
        ...
        if (!equal.equal(newObservableQueryOptions, this.previousData.observableQueryOptions)) {
        ...
        this.currentSubscription = this.currentObservable.subscribe({
            next: function (_a) {
                ...
                if (previousResult &&
                    ...
                    equal.equal(previousResult.data, data)) {
                    return;
                }
            ...
            error: function (error) {
                ...
                if ((previousResult && previousResult.loading) ||
                    !equal.equal(error, _this.previousData.error)) {
    ...
    QueryData.prototype.handleErrorOrCompleted = function (_a) {
        ...
        if (!loading) {
            ...
            if (this.previousOptions &&
                ...
                equal.equal(this.previousOptions.query, query) &&
                equal.equal(this.previousOptions.variables, variables)) {
                return;
            }
...
function useDeepMemo(memoFn, key) {
    ...
    var ref = useRef();
    if (!ref.current || !equal.equal(key, ref.current.key)) {
...
var MutationData = (function (_super) {
    ...
    MutationData.prototype.updateResult = function (result) {
        if (this.isMounted &&
            (!this.previousResult || !equal.equal(this.previousResult, result))) {
    ...
    SubscriptionData.prototype.execute = function (result) {
        ...
        if (shouldResubscribe !== false &&
            ...
            (this.previousOptions.subscription !== this.getOptions().subscription ||
                !equal.equal(this.previousOptions.variables, this.getOptions().variables) ||
...
function useApolloClient() {
    ...
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(client, 8) : tsInvariant.invariant(client, 'No Apollo Client instance can be found. Please ensure that you ' +
